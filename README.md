...
**LOLAnalyzer** は、Discord 上で動作する League of Legends (LoL) のランク情報追跡・分析ボットです。
...
- **定期レポート**: 指定した時間・チャンネルに、日次/週次などのランク変動レポートを自動送信します（表形式・グラフ形式に対応）。
...
- `/schedule add` : 新しい通知スケジュールを作成します（対話形式）。
  - 入力形式: `時間(HH:MM) チャンネル 期間(日) 出力形式(table/graph)`
  - 例: `21:00 here 7 table` （毎日21時に、このチャンネルへ過去7日間の表を送信）
...
### 分析・レポート
- `/graph` : 指定ユーザーのランク推移グラフを表示します。
  - 引数: `riot_id`（ユーザー名または 'all'）, `period`（daily/weekly/monthly）, `force_fetch`（最新取得）
- `/report` : 指定期間の集計レポート画像を生成します。
  - 引数: `days`（日数）, `riot_id`（特定ユーザーのみ表示する場合）
